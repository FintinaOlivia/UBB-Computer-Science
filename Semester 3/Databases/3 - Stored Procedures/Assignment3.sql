...
create table CharLocations (
  CharlocID INT PRIMARY KEY,
  LocationName varchar(255)
);
...
-- one statement must violate referential integrity constraints;
INSERT INTO Characters(CharacterID, PlayerID, CharacterName, Race, Class, Level, Attributes)
VALUES
  (1,2,'Seraphina Dawnbringer', 'Aasimar', 'Monk', 2, 'Strength: 10, Dexterity: 14, Constitution: 10, Intelligence: 12, Wisdom: 14, Charisma: 16');
...
-- Query displays all characters with CharacterID < 5 and 
-- CharacterID > 5 and < 8, ID belongs to the interval
-- [1, 4] U [6, 7]
SELECT * FROM Characters
WHERE CharacterID < 5
UNION ALL
...
-- Query displays all items that are weapons or armor
SELECT * FROM Items
...
-- Query displays all characters that have participated in adventure 6
SELECT * FROM Characters
WHERE CharacterID IN (
  SELECT CharacterID FROM CharacterAdventure WHERE AdventureID = 6);

-- Query displays all players that have characters
SELECT 
  Characters.PlayerID
FROM Characters
INTERSECT
...
-- Query displays all characters that have not participated in any adventure
SELECT * FROM Characters
WHERE CharacterID NOT IN (SELECT CharacterID FROM CharacterAdventure);

-- Query displays all players whose ID is lesser than 5
SELECT PlayerName
FROM Players
EXCEPT
SELECT PlayerName
...
-- Query displays all characters that have participated in an adventure and
-- have completed a quest (have character achievements)
-- tables joined: Characters, CharacterAdventure, CharacterAchievements
-- many-to-many relationships: CharacterAdventure, CharacterAchievements
SELECT 
...
-- Left join displays all characters and their items
SELECT * FROM Adventures
LEFT JOIN Campaigns ON Adventures.CampaignID = Campaigns.CampaignID

-- Right join displays all NPCs and their locations
SELECT * FROM NPCs
RIGHT JOIN Locations ON NPCs.LocationID = Locations.LocationID

-- Full join displays all items and their owners
SELECT * FROM Items
...
-- Query displays all characters that have participated in an adventure
-- that belongs to a campaign with a specific name (Campaign 1)
SELECT * FROM Characters
WHERE CharacterID IN (
...
-- Query displays all items that belong to characters that belong to a player
SELECT * FROM Items
WHERE Owner IN (
...
-- Query displays all items that belong to characters 
...
-- Query displays the number of class types and types, ordered
-- by the number of characters that have that class, descending
SELECT 
...
-- Query displays the number of characters per player, displaying only those
-- who have more than one character
SELECT 
...
-- Query displays the number of NPCs per location, ordered decreasing
-- It only displays the locations that have NPCs with level higher than 3
SELECT
...
-- Characters that have a higher level than any of the characters owned
-- by the first player
SELECT * FROM Characters
WHERE Level > ANY (
  SELECT Level FROM Characters WHERE PlayerID = 1)

-- Locations that have NPCs with a higher level than 3
SELECT * FROM Locations
...
-- Query with arithmetic operators in the Select Statement
SELECT
  Characters.Level * 10 AS 'Character Power'
FROM Characters
GROUP BY Characters.Level

-- Query with IN operator in an Update Statement
UPDATE Characters 
SET CharacterName = 'Anana Banana' --
WHERE CharacterID IN (
  SELECT CharacterID FROM Characters WHERE CharacterName = 'Sir Aldric the Brave')

SELECT * FROM Characters

-- Lab 3 : 
-- Sometimes, after you design a database, you need to change its structure.
--  Unfortunately, changes aren’t correct every time, so they must be reverted. 
...
CREATE OR ALTER PROCEDURE setCharacterLevelFromINTtoTINY AS
  BEGIN
    ALTER TABLE Characters ALTER COLUMN Level TINYINT
    PRINT 'Changed column type from INT to VARCHAR'
  END
GO

-- a. undo the modification
CREATE OR ALTER PROCEDURE revertCharacterLevelFromTINYtoINT AS
  BEGIN
    ALTER TABLE Characters ALTER COLUMN Level INT
    PRINT 'Changed column type from VARCHAR to INT'
  END
GO

-- b. add / remove a column
CREATE OR ALTER PROCEDURE addCharacterAgeColumn AS
  BEGIN
    ALTER TABLE Characters ADD Age INT
    PRINT 'Added Age column'
  END
GO

CREATE OR ALTER PROCEDURE removeCharacterAgeColumn AS
  BEGIN
    ALTER TABLE Characters DROP COLUMN Age
    PRINT 'Removed Age column'
  END
GO

-- c. add / remove a DEFAULT constraint
CREATE OR ALTER PROCEDURE addCharacterLevelDefaultConstraint AS
  BEGIN
    ALTER TABLE Characters ADD CONSTRAINT DF_CharacterLevel DEFAULT 1 FOR Level
    PRINT 'Added default constraint for Level column'
  END
GO

CREATE OR ALTER PROCEDURE removeCharacterLevelDefaultConstraint AS
  BEGIN
    ALTER TABLE Characters DROP CONSTRAINT DF_CharacterLevel
    PRINT 'Removed default constraint for Level column'
  END
GO

-- g. create / drop a table
CREATE OR ALTER PROCEDURE createCharactersBackupTable AS
  BEGIN
     create table CharactersBackup (
        Name VARCHAR(100) NOT NULL,
        Role varchar(100) NOT NULL,
        Level INT,
        LocationID INT,
        constraint character_primary primary key(Name)
     )
    PRINT 'Created CharactersBackup table'
  END
GO

CREATE OR ALTER PROCEDURE dropCharactersBackupTable AS
  BEGIN
    DROP TABLE CharactersBackup
    PRINT 'Dropped CharactersBackup table'
  END
GO

-- d. add / remove a primary key
CREATE OR ALTER PROCEDURE removePrimaryKeyBackupTable AS
  BEGIN
    ALTER TABLE CharactersBackup DROP CONSTRAINT character_primary
    PRINT 'Removed primary key for CharactersBackup table'
  END
GO

CREATE OR ALTER PROCEDURE addPrimaryKeyBackupTable AS
  BEGIN
    ALTER TABLE CharactersBackup 
    ADD CONSTRAINT character_primary PRIMARY KEY (Name)
    PRINT 'Added primary key for CharactersBackup table'
  END
GO

-- e. add / remove a candidate key
CREATE OR ALTER PROCEDURE addCharacterNameCandidateKey AS
  BEGIN
    ALTER TABLE Characters ADD CONSTRAINT CharacterNameCandidateKey UNIQUE (CharacterName)
    PRINT 'Added candidate key for CharacterName column'
  END
GO

CREATE OR ALTER PROCEDURE removeCharacterNameCandidateKey AS
  BEGIN
    ALTER TABLE Characters DROP CONSTRAINT CharacterNameCandidateKey
    PRINT 'Removed candidate key for CharacterName column'
  END
GO

-- f. add / remove a foreign key
CREATE OR ALTER PROCEDURE addCharacterBackupForeignKEY AS
  BEGIN
    ALTER TABLE CharactersBackup
    ADD CONSTRAINT character_backup_fk FOREIGN KEY (LocationID) REFERENCES Locations(LocationID)
    PRINT 'Added foreign key for CharactersBackup table'
  END
GO

CREATE OR ALTER PROCEDURE removeCharacterBackupForeignKEY AS
  BEGIN
    ALTER TABLE CharactersBackup DROP CONSTRAINT character_backup_fk
    PRINT 'Removed foreign key for CharactersBackup table'
  END
GO

-- Remove all stored procedures
EXEC removeCharacterNameCandidateKey
EXEC removeCharacterLevelDefaultConstraint


-- Testing the stored procedures
EXEC setCharacterLevelFromINTtoTINY
EXEC revertCharacterLevelFromTINYtoINT

EXEC addCharacterAgeColumn
EXEC removeCharacterAgeColumn

EXEC addCharacterLevelDefaultConstraint
EXEC removeCharacterLevelDefaultConstraint

EXEC removePrimaryKeyBackupTable
EXEC addPrimaryKeyBackupTable

EXEC addCharacterNameCandidateKey
EXEC removeCharacterNameCandidateKey

EXEC addCharacterBackupForeignKEY
EXEC removeCharacterBackupForeignKEY

EXEC dropCharactersBackupTable
EXEC createCharactersBackupTable
SELECT * FROM CharactersBackup
...
('setCharacterLevelFromINTtoTINY', 'revertCharacterLevelFromTINYtoINT', 1),
('addCharacterAgeColumn', 'removeCharacterAgeColumn', 2),
('addCharacterLevelDefaultConstraint', 'removeCharacterLevelDefaultConstraint', 3),
('addPrimaryKeyBackupTable', 'removePrimaryKeyBackupTable', 4),
('addCharacterNameCandidateKey', 'removeCharacterNameCandidateKey', 5),
('addCharacterBackupForeignKEY', 'removeCharacterBackupForeignKEY', 6),
('createCharactersBackupTable', 'dropCharactersBackupTable', 7)
...
BEGIN
  ...
  -- parameter validity check
	IF @version < 0 OR @version > 7   
		BEGIN
			RAISERROR('Snow White and the 7 versions : please choose a valid version!',17,1)
			RETURN
		END
	ELSE
    -- check if we got to the desired version
			...
			BEGIN
				WHILE @currentVersion < @version
					BEGIN
						...
						SET @currentProcedure = (SELECT Current_Procedure
			...
			BEGIN
				WHILE @currentVersion > @version
					BEGIN
						...
						SET @currentProcedure = (SELECT Previous_Procedure
												FROM VersioningTable
